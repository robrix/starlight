...
updatePosition :: Num a => Actor a -> Actor a
updatePosition a@Actor{ position, velocity } = a { Actor.position = position .+^ velocity }

applyGravity :: Delta Seconds Float -> Float -> StateVectors Float -> Actor Float -> Actor Float
...
runAction (Delta (Seconds dt)) = \case
  Thrust -> do
    rotation <- use (rotation_ @(Actor Float))
    velocity_ @(Actor Float) += rotate rotation (unit _x ^* thrust) ^. _xy
  Face dir -> do
    velocity <- use (velocity_ @(Actor Float))
    direction <- case dir of
      Forwards  -> pure (Just velocity)
      Backwards -> pure (Just (-velocity))
      Target    -> do
        system <- ask @(System StateVectors Float)
        target   <- use (target_ @Float)
        position <- use (position_ @(Actor Float))
        pure ((^. position_ . to (unP . flip direction position)) <$> (target >>= (system !?)))
    maybe (pure ()) (modifying (rotation_ @(Actor Float)) . face angular . angleOf) direction
  Turn t -> rotation_ @(Actor Float) *= axisAngle (unit _z) (getRadians (case t of
    L -> angular
    R -> -angular))
  Fire Main -> pure ()
  ChangeTarget change -> do
    identifiers <- view @(System StateVectors Float) (to identifiers)
    target_ @Float %= case change of
