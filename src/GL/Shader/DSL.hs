...
import           GL.Shader.Expr hiding (var)
import qualified GL.Shader.Expr as Expr (var)
...
data Shader (u :: (Type -> Type) -> Type) (i :: (Type -> Type) -> Type) (o :: (Type -> Type) -> Type) where
  Shader :: Vars u => ((forall k . u (Render k)) -> Stage i o) -> Shader u i o

program :: Vars u => ((forall k . u (Render k)) -> Stage i o) -> Shader u i o
program = Shader
...
data Stage i o where
  ...
  V :: (Vars i, Vars o) => (i (Render 'Shader.Vertex)   -> o (Ref 'Shader.Vertex)   -> Decl 'Shader.Vertex   ()) -> Stage i o
  G :: (Vars i, Vars o) => (i (Render 'Shader.Geometry) -> o (Ref 'Shader.Geometry) -> Decl 'Shader.Geometry ()) -> Stage i o
  F :: (Vars i, Vars o) => (i (Render 'Shader.Fragment) -> o (Ref 'Shader.Fragment) -> Decl 'Shader.Fragment ()) -> Stage i o

vertex   :: (Vars i, Vars o) => (i (Render 'Shader.Vertex)   -> o (Ref 'Shader.Vertex)   -> Decl 'Shader.Vertex   ()) -> Stage i o
vertex = V

geometry :: (Vars i, Vars o) => (i (Render 'Shader.Geometry) -> o (Ref 'Shader.Geometry) -> Decl 'Shader.Geometry ()) -> Stage i o
geometry = G

fragment :: (Vars i, Vars o) => (i (Render 'Shader.Fragment) -> o (Ref 'Shader.Fragment) -> Decl 'Shader.Fragment ()) -> Stage i o
fragment = F
...
stageSources u = \case
  ...
  where
  renderStage :: (Vars i, Vars o) => Shader.Stage -> (i (Render k) -> o (Ref k) -> Decl k ()) -> (Shader.Stage, Doc ())
  renderStage t f = (,) t
